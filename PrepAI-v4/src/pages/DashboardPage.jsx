...
export default function DashboardPage({ history, onStart, onHome, onMNC, onProfile, onLogout }) {
  ...
  const avg = hasHistory
    ? Math.round(history.reduce((s, h) => s + h.avgScore, 0) / history.length)
    : 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <div className="resp-padding" style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px', animation: 'fadeUp .4s ease' }}>
        ...
        {/* Summary metrics */}
        <div className="resp-wrap" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, marginBottom: 24 }}>
          {[
            ['Sessions', history.length, null],
            ['Avg Score', avg ? avg + '%' : '—', avg ? getColor(avg) : null],
            ['Best Score', best ? best + '%' : '—', best ? getColor(best) : null],
            ['Qual. MNCs', qualifiedCount + '/30', avg >= 60 ? 'var(--success)' : 'var(--danger)', onMNC],
            ['Streak', streak.count + ' days', 'var(--warning)'],
          ].map(([label, value, color, onClick]) => (
            <div key={label} onClick={onClick} style={{
              background: 'var(--bg-card)', border: '1px solid var(--border)',
              borderRadius: 14, padding: '20px 16px', textAlign: 'center',
              cursor: onClick ? 'pointer' : 'default', transition: 'all .2s',
              boxShadow: onClick ? 'var(--shadow-card)' : 'none',
        ...
        {hasHistory ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={recentTrend} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                      ...
                      <Line
                        ...
                        stroke="var(--accent-a)" strokeWidth={2.5}
                        dot={{ fill: 'var(--accent-a)', r: 4, strokeWidth: 0 }}
                        activeDot={{ r: 6 }}
                      />
                    </LineChart>
                  </ResponsiveContainer>
          ...
          <div style={{
            ...
            <div style={{
              fontFamily: 'var(--font-display)', fontSize: 20, color: 'var(--text-primary)',
              marginBottom: 10,
            }}>No sessions yet</div>
